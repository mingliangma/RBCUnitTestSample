...
class WriteReadSpec extends FlatSpec with EmbeddedCassandra with SparkTemplate with Matchers{
  ...
  it should "be able to return a String" in {
    var testValue = WriteRead.etlSampleFunction1()
    testValue should equal("test")
  }

  it should "be able to return a function" in {
    var testValue = WriteRead.etlSampleFunction2()
    testValue should equal(List("the", "quick", "brown", "fox", "jumps", "over", "the", "lazy", "dog"))
  }
